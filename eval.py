...
def evaluate(beam_size):
    ...
    # For each image
    for i, (image, caps, caplens, allcaps) in tqdm(enumerate(loader)):
            #loader, desc="EVALUATING AT BEAM SIZE " + str(beam_size)):
        ...
        while True:
            ...
            # Convert tensor indices to integer tensors
        ...
        bleu1_score = sentence_bleu(reference, hypothese, weights=(1, 0, 0, 0))
        bleu1.append(bleu1_score)
        # Calculate BLEU-2 score
        bleu2_score = sentence_bleu(reference, hypothese, weights=(0.5, 0.5, 0, 0))
        bleu2.append(bleu2_score)
        # Calculate BLEU-3 score
        bleu3_score = sentence_bleu(reference, hypothese, weights=(0.33, 0.33, 0.33, 0))
        bleu3.append(bleu3_score)
        # Calculate BLEU-4 score
        bleu4_score = sentence_bleu(reference, hypothese)
        bleu4.append(bleu4_score)

        assert len(references) == len(hypotheses)

    # Path to the pickle file
    pickle_bleu1 = 'bleu1.pkl'
    pickle_bleu2 = 'bleu2.pkl'
    pickle_bleu3 = 'bleu3.pkl'
    pickle_bleu4 = 'bleu4.pkl'

    # Pickle the list
    with open(pickle_bleu1, 'wb') as f:
        pickle.dump(bleu1, f)
    with open(pickle_bleu2, 'wb') as f:
        pickle.dump(bleu2, f)
    with open(pickle_bleu3, 'wb') as f:
        pickle.dump(bleu3, f)
    with open(pickle_bleu4, 'wb') as f:
        pickle.dump(bleu4, f)

    # Calculate BLEU-4 scores
    bleu4_corpus = corpus_bleu(references, hypotheses)
